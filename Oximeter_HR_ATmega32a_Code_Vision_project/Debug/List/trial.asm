...
_0x0:
	.DB  0x42,0x50,0x4D,0x3A,0x20,0x0
_0x2000003:
	.DB  0x80,0xC0
_0x2040060:
	.DB  0x1
...
; 0000 005B Sinewave[i] = float2fix(sin(6.283 * ((float)i) / N_WAVE));
	MOV  R30,R17
	LDI  R26,LOW(_Sinewave)
	LDI  R27,HIGH(_Sinewave)
	LDI  R31,0
	CALL SUBOPT_0x0
	PUSH R31
	PUSH R30
...
; 0000 00A4 heartsamples[samplenumber] = currentvalue;
	MOV  R30,R6
	LDI  R31,0
	SUBI R30,LOW(-_heartsamples)
	SBCI R31,HIGH(-_heartsamples)
	ST   Z,R5
...
; 0000 00AF FFTfix(heartsamplescopy, zeros, LOG2_N_WAVE);
	LDI  R30,LOW(_heartsamplescopy)
	LDI  R31,HIGH(_heartsamplescopy)
	ST   -Y,R31
	ST   -Y,R30
	LDI  R30,LOW(_zeros)
	LDI  R31,HIGH(_zeros)
...
SUBOPT_0x8:
	MOV  R30,R17
	LDI  R26,LOW(_heartsamplescopy)
	LDI  R27,HIGH(_heartsamplescopy)
	LDI  R31,0
	RJMP SUBOPT_0x5
